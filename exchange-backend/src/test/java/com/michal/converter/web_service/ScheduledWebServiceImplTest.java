...
class ScheduledWebServiceImplTest {
    ...
    @Test
    void getExchangeRate_whenRateMapNotHaveCurrency_thenItMakeRequestAndSaves() {
        setRates();
        String currency = "usd";

        when(apiRequest.exchangeRateApiRequest(anyString())).thenReturn(BigDecimal.ONE);

        underTest.getExchangeRate(currency);

        assertThat(rates).isNotEmpty();
        assertThat(rates.get(currency).compareTo(BigDecimal.ONE)).isEqualTo(0);
    }

    @Test
    void getExchangeRate_whenRateMapHaveCurrency_thenMethodReturns() {
        String currency = "usd";
